...
def make_figure(group_dirs, outpath):
    ...
    cax = sns.heatmap(
        Z_euclid_ord,
        ax=axA,   # oppure il tuo asse reale
        cmap='RdBu_r',
        center=0,
        xticklabels=True,
        yticklabels=True,
    ...
    # 🔥 recupera la colorbar dal return value (NON da ax)
    ...
    axB.set_ylabel("Cortex Spearman correlation/ Cosine similarity (Z)", fontsize=16)
    axB.set_title("B. Correlation between similarity measures", fontsize=20)
    ...
    # Axes
    axD.set_xlabel("Cortex Euclidean similarity (raw)", fontsize=16)
    axD.set_ylabel("Subcortex Euclidean similarity (raw)", fontsize=16)
    axD.set_title("C. Correlation between cortex and subcortex", fontsize=20)
    ...
    axC.set_xticklabels(sud_names, ha='right', fontsize=18)
    axC.set_ylabel('Euclidean similarity (Z)', fontsize=18)
    axC.legend(
    frameon=False,
    loc='lower right',              # posizione relativa al bbox
    bbox_to_anchor=(1, 0),          # x=1 (destra), y=0 (sotto)
    fontsize=16)                    # dimensione del testo della legenda
    axC.set_title('D. PSY cluster fingerprints', fontsize=20)
    axC.grid(False)
