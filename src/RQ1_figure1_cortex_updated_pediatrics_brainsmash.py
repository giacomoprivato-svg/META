...
Adolescents-only cortex data
EUCLIDEAN similarity version (permutation-calibrated Z-values)

BRAINSMASH VERSION:
- identical plotting/layout/code to the pspin adolescent figure
...
def make_figure(group_dirs, outpath):
    ...
    raw_cortex_list = []
    for gd in group_dirs:
        raw_cortex_list.append(
            pd.read_csv(os.path.join(gd, "RAW_cortex_euclidean.csv"), index_col=0)
        )

    raw_cortex = pd.concat(raw_cortex_list, axis=0).loc[psy_names]

    raw_subctx = pd.read_csv(
        os.path.join(repo_dir, "ALL_outputs_RQ1", "adolescents_all", "RAW_subctx_euclidean.csv"),
        index_col=0
    ).reindex(psy_names)  # <-- FIX QUI
    ...
    axA.set_title("A. PSY × SUD Euclidean similarity (Z)", fontsize=18)
    axA.grid(False)
    ...
    manual_clusters = {
        ...
        'Neurodevelopmental': ['ASD','ADHD_ch', 'ADHD_ado' ],
        'AN/OCD': ['AN','OCD'],
    ...
    # --- FIX NaN (importantissimo) ---
    mask = ~np.isnan(x_vals) & ~np.isnan(y_vals)
    x_vals = x_vals[mask]
    y_vals = y_vals[mask]
    psy_labels = psy_labels[mask]
...
if __name__ == "__main__":
    ...
    make_figure(adults_dirs, "Figure1_Euclidean_final_brainsmash.png")