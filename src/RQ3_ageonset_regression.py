...
OUTFIG_COMBINED = os.path.join(BASE_DIR, "overlap_vs_age_peak_meanPSY.png")
...
# -----------------------
def merge_age(df):
    df["PSY_for_age"] = df["PSY"].replace({
        "ADHD_ch": "ADHD",
        "ADHD_ado": "ADHD"
    })

    df = pd.merge(df, age_onset,
                  left_on="PSY_for_age",
                  right_on="PSY",
                  how="left")
...
def plot_on_ax(ax, df, title):
    ...
    for psy_label in df["PSY_main"].unique():
        ...
        ax.scatter(sub["Peak"], sub["Similarity"],
                   s=60, alpha=0.6,
                   color=PSY_COLORS.get(base_psy, "grey"),
                   edgecolor='none')
    ...
    for _, row in df_mean.iterrows():
        color = PSY_COLORS.get(row["PSY_base"], "grey")

        ax.scatter(row["Peak"], row["Similarity"],
                   s=200,
                   edgecolor='black',
                   linewidth=1.5,
                   color=color,
    ...
    # REGRESSION SU MEDIE
    # -----------------------
    df_mean["logPeak"] = np.log(df_mean["Peak"])

    p_lin_text = "NA"
    p_quad_text = "NA"

    if df_mean.shape[0] >= 3 and np.ptp(df_mean["logPeak"]) > 0:

        # Linear
        X_lin = sm.add_constant(df_mean["logPeak"])
        model_lin = sm.OLS(df_mean["Similarity"], X_lin).fit()

        p_lin = model_lin.pvalues.get("logPeak", np.nan)
        p_lin_text = f"p < 0.001" if p_lin < 0.001 else f"p={p_lin:.3f}"

        # Quadratic
        df_mean["logPeak2"] = df_mean["logPeak"]**2
        X_quad = sm.add_constant(df_mean[["logPeak","logPeak2"]])
        model_quad = sm.OLS(df_mean["Similarity"], X_quad).fit()

        p_quad = model_quad.pvalues.get("logPeak2", np.nan)
        p_quad_text = f"p < 0.001" if p_quad < 0.001 else f"p={p_quad:.3f}"

        xs = np.linspace(df_mean["Peak"].min(), df_mean["Peak"].max(), 100)

        ys_lin = model_lin.predict(sm.add_constant(np.log(xs)))
        ax.plot(xs, ys_lin, color="black", lw=2, linestyle="--", alpha=0.3)

        ys_quad = model_quad.predict(
    ...
    ax.set_ylabel("Neuroanatomical similarity", fontsize=16)
