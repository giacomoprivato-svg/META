...
# =====================================================
# NOW:
# higher values = MORE atrophy
# lower values = LESS atrophy
shared = -shared

# =====================================================
# MICA GRADIENTS
# =====================================================
# NOTE: correlations with the transcriptional components (C1/C2/C3) are NOT
# computed here. Per the Methods, transcriptional axes are derived from
# diffusion map embedding (DME), not PCA, and that analysis lives in
# RQ2_corr_C1C2C3.py (loading ahba_dme_scores_in_dk.csv). This script is
# dedicated to the functional-connectivity (FC) and microstructural-profile
# covariance (MPC) cortical gradients only.
fc_grad = pd.read_csv(PATH_FC_GRAD, header=None).iloc[:,0].to_numpy()
mpc_grad = pd.read_csv(PATH_MPC_GRAD, header=None).iloc[:,0].to_numpy()

print("FC gradient:", fc_grad.shape)
print("MPC gradient:", mpc_grad.shape)
...
# RUN ALL
# =====================================================
compute_all(fc_grad, shared, "FC_GRADIENT", "ATROPHY")
compute_all(mpc_grad, shared, "MPC_GRADIENT", "ATROPHY")
...
for vals, name in zip(
    [fc_grad, mpc_grad],
    ["FC_gradient_map", "MPC_gradient_map"]
):
