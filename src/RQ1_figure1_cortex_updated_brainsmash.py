...
BRAINSMASH VERSION:
- identical plotting/layout/code
- ONLY difference: loads PVAL files from BrainSMASH outputs
"""
...
def make_figure(group_dirs, outpath):
    ...
    # ---- Load p-values FROM BRAINSMASH OUTPUTS ----
    ...
    for gd in group_dirs:
        PVAL_list.append(
            read_csv_safely(os.path.join(gd, "PVAL_cortex_euclidean_brainsmash.csv"))
        )
    ...
    for j in range(PVAL.shape[1]):
        ...
        col_name = PVAL.columns[j]

        print(f"\n==============================")
        print(f"FDR results — column: {col_name}")
        print(f"==============================")

        for i, (row_name, p) in enumerate(zip(PVAL.index, p_fdr)):
            print(f"{row_name:20s}  pFDR = {p:.6g}")
...
if __name__ == "__main__":
    ...
    make_figure(
        adults_dirs,
        "Figure1_Euclidean_final_brainsmash.png"
    )