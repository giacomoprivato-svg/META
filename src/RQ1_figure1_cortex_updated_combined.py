...
COMBINED-SIGNIFICANCE VERSION:
- same plotting/layout as the BrainSMASH figure
- asterisks in Panel A are drawn ONLY where a pair survives FDR (per column)
  under BOTH the spin and the BrainSMASH null models (intersection)
- larger PSY/SUD tick labels and larger asterisks
"""
...
# ---------------------------
FONT_TICKS = 26   # PSY/SUD tick labels in the heatmap  [era 20]
FONT_AST   = 34   # asterisks                            [era 20]
...
def make_figure(group_dirs, outpath):
    ...
    # ---- Load p-values: BrainSMASH + spin ----
    def read_csv_safely(path):
        df = pd.read_csv(path, index_col=0, sep=None, engine="python")
        return df.apply(pd.to_numeric, errors='coerce')

    PVAL_bs_list, PVAL_spin_list = [], []
    for gd in group_dirs:
        PVAL_bs_list.append(read_csv_safely(os.path.join(gd, "PVAL_cortex_euclidean_brainsmash.csv")))
        PVAL_spin_list.append(read_csv_safely(os.path.join(gd, "PVAL_cortex_euclidean.csv")))

    PVAL_bs = pd.concat(PVAL_bs_list, axis=0).loc[mask_psy]
    PVAL    = pd.concat(PVAL_spin_list, axis=0).loc[mask_psy]

    # align both p-value matrices to the Z layout (defensive)
    PVAL    = PVAL.reindex(index=Z_euclid.index, columns=Z_euclid.columns)
    PVAL_bs = PVAL_bs.reindex(index=Z_euclid.index, columns=Z_euclid.columns)
    ...
    raw_cortex = pd.read_csv(
        os.path.join(repo_dir, "ALL_outputs_RQ1", "adults_all", "RAW_cortex_euclidean.csv"),
        index_col=0
    ).loc[psy_names]

    raw_subctx = pd.read_csv(
        os.path.join(repo_dir, "ALL_outputs_RQ1", "adults_all", "RAW_subctx_euclidean.csv"),
        index_col=0
    ).loc[psy_names]
    ...
    # ------------------------------------------------
    # PANEL A — Heatmap with asterisks (spin ∩ BrainSMASH)
    # ------------------------------------------------
    cax = sns.heatmap(
        Z_euclid_ord,
        ax=axA,
        ...
        xticklabels=True,
        yticklabels=True,
        vmin=Z_euclid_ord.values.min(),
        vmax=6,
        cbar_kws={'label': 'Euclidean similarity (Z)', 'shrink':0.5, 'pad':0.01}
    )

    cbar = cax.collections[0].colorbar

    cbar.set_label('Euclidean similarity (Z)', fontsize=18)
    cbar.ax.tick_params(labelsize=14)
    axA.tick_params(axis='both', labelsize=FONT_TICKS)

    # Asterischi più grandi (solo dove passano ENTRAMBI i null)
    ...
    axA.set_title("A. PSY × SUD Euclidean similarity (Z)", fontsize=20)
    axA.grid(False)
    axA.set_aspect('equal')

    # ------------------------------------------------
    # PANEL B — Euclidean vs Spearman/Cosine
    # ------------------------------------------------
    manual_clusters = {
    ...
    axB.plot(
        x_fit,
        coeff_spearman[0]*x_fit + coeff_spearman[1],
        color="#469fdf",
        lw=2,
        alpha=0.8
    )
    ...
    axB.plot(
        x_fit,
        coeff_cosine[0]*x_fit + coeff_cosine[1],
        color="#e07270",
        lw=2,
        alpha=0.8
    )

    axB.set_xlabel("Cortex Euclidean similarity (Z)", fontsize=16)

    axB.set_ylabel(
        "Cortex Spearman correlation/ Cosine similarity (Z)",
        fontsize=16
    )

    axB.set_title(
        "B. Correlation between similarity measures",
        fontsize=20
    )
    ...
    # ------------------------------------------------
    # PANEL C — Cortex vs Subcortex RAW
    # ------------------------------------------------
    x_vals = raw_cortex.values.flatten()
    y_vals = raw_subctx.values.flatten()

    psy_labels = np.repeat(
        raw_cortex.index.values,
        raw_cortex.shape[1]
    )
    ...
    colors = [
        cluster_colors.get(cluster_map.get(d, None), "black")
        for d in psy_labels
    ]
    ...
    for cname, color in cluster_colors.items():
        ...
        inds = [
            i for i, d in enumerate(psy_labels)
            if cluster_map.get(d) == cname
        ]
    ...
    axD.set_xlabel(
        "Cortex Euclidean similarity (raw)",
        fontsize=16
    )

    axD.set_ylabel(
        "Subcortex Euclidean similarity (raw)",
        fontsize=16
    )

    axD.set_title(
        "C. Correlation between cortex and subcortex",
        fontsize=20
    )
...
if __name__ == "__main__":
    ...
    make_figure(
        adults_dirs,
        "Figure1_Euclidean_final_combined.png"
    )