...
- **Comorbidity is exploratory.** The workbook is still keyed on PTSD, and no
  ...
  rows. Reported descriptively, no figure in the main text.
...
- **A null on C1 is cancellation, not absence.** The adult PSY and SUD
  ...
  of `RQ2_gradients.py`) and the mean cancels them.
  `RQ2_shared_gradients_figure.py` draws both components for this reason — the
  connector between them turns red whenever they straddle zero.
