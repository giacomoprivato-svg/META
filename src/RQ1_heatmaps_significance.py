#!/usr/bin/env python3
"""
Figure generator — RAW and Z PSY × SUD similarity (cortex only)

For each age group (adults, adolescents), creates:
- Figure 1: RAW values (5 heatmaps)
- Figure 2: Z values (5 heatmaps)

Last two panels show Euclidean distance:
- masked by significance
- masked by FDR-corrected significance
"""
...
def two_tailed_perm_test(true_vals, null_vals):
    ...
    z_vals = norm.isf(p_vals / 2)
    z_vals[p_lower < p_upper] *= -1

    return z_vals, p_vals


def load_and_concat(group_dirs, fname):
    dfs = []
...
def make_figures(group_dirs, out_prefix, group_label):
    ...
    # ---- Load RAW and Z ----
    raw = {
        "spearman": load_and_concat(group_dirs, "RAW_cortex_spearman.csv"),
        "cosine": load_and_concat(group_dirs, "RAW_cortex_cosine.csv"),
        "euclidean": load_and_concat(group_dirs, "RAW_cortex_euclidean.csv"),
    }

    z = {
        "spearman": load_and_concat(group_dirs, "Z_cortex_spearman.csv"),
        "cosine": load_and_concat(group_dirs, "Z_cortex_cosine.csv"),
        "euclidean": load_and_concat(group_dirs, "Z_cortex_euclidean.csv"),
    }
    ...
    for s in sud_names:
        null_df = load_and_concat(group_dirs, f"NULLS_cortex_euclidean_{s}.csv")
        nulls.append(null_df.values)

    nulls = np.stack(nulls, axis=1)  # (n_psy, n_sud, n_perm)
    ...
    for j in range(n_sud):
        _, p_tmp = two_tailed_perm_test(true_vals[:, j], nulls[:, j, :])
        p_sig[:, j] = p_tmp

    sig_mask = p_sig < 0.05

    p_fdr = np.zeros_like(p_sig)
    for j in range(n_sud):
        _, p_fdr[:, j] = fdrcorrection(p_sig[:, j])

    fdr_mask = p_fdr < 0.05

    # ---- Clustering (from Z euclidean) ----
    ...
    # ---- Plotting ----
    def plot_figure(mats, masks, fname, is_z):
        ...
        # ---- Top row: unmasked (DataFrames → labels preserved) ----
        ...
        # ---- Bottom row: masked Euclidean ----
        for i, (mask, title) in enumerate(
            [(sig_mask, "D. Euclidean (p < 0.05)"),
        ...
        fig.suptitle(
            f"{group_label} — Cortex only — {value_label}",
            fontsize=18
        )
...
if __name__ == "__main__":
    ...
    adolescents_dirs = [
        os.path.join(base, "adolescents_all"),
        os.path.join(base, "adolescents_ctx"),
    ]
